...
"""
import numpy as np
import rationalcubic as rc
import normaldistribution as nd
import erf_cody as ec

ONE_OVER_SQRT_TWO = nd.ONE_OVER_SQRT_TWO
ONE_OVER_SQRT_TWO_PI = nd.ONE_OVER_SQRT_TWO_PI
SQRT_TWO_PI = nd.SQRT_TWO_PI
...
FOURTH_ROOT_DBL_EPSILON = np.sqrt(SQRT_DBL_EPSILON)
EIGHTH_ROOT_DBL_EPSILON = np.sqrt(FOURTH_ROOT_DBL_EPSILON)
SIXTEENTH_ROOT_DBL_EPSILON = np.sqrt(EIGHTH_ROOT_DBL_EPSILON)
SQRT_DBL_MIN = np.sqrt(DBL_MIN)
SQRT_DBL_MAX = np.sqrt(DBL_MAX)
...
def is_below_horizon(x):
    ...
    return np.fabs(x) < DENORMALIZATION_CUTOFF


def implied_volatility_output(count, volatility):
...
def normalised_intrinsic(x, q):
    ...
    if (x2 < 98 * FOURTH_ROOT_DBL_EPSILON):
        if q < 0:
            ...
            phi = 1
        return np.fabs(max(phi*x*(1+x2*((1.0/24.0)+x2*((1.0/1920.0)+x2*((1.0/322560.0)+(1.0/92897280.0)*x2)))), 0.0))


def normalised_intrinsic_call(x):
...
def normalised_black_call_using_erfcx(h, t):
    ...
    #
    b = 0.5 * np.exp(-0.5*(h*h+t*t)) * (ec.erfcx_cody(-ONE_OVER_SQRT_TWO*(h+t)) - ec.erfcx_cody(-ONE_OVER_SQRT_TWO*(h-t)))
    return np.fabs(max(b, 0.0))
...
def small_t_expansion_of_normalized_black_call(h, t):
    ...
    a = 1+h*(0.5*SQRT_TWO_PI)*ec.erfcx_cody(-ONE_OVER_SQRT_TWO*h)
    w = t*t
    h2 = h*h
...
def normalized_black_call_using_norm_cdf(x, s):
    ...
    t = 0.5*s
    b_max = np.exp(0.5*x)
    b = nd.norm_cdf(h + t) * b_max - nd.norm_cdf(h - t) / b_max
...
def normalised_vega(x, s):
    ax = np.fabs(x)
    if ax <= 0:
        return ONE_OVER_SQRT_TWO_PI*np.exp(-0.125*s*s)
    else:
        if ((s <= 0) or (s <= ax*SQRT_DBL_MIN)):
            return 0
        else:
            return ONE_OVER_SQRT_TWO_PI*np.exp(-0.5*(square(x/s)+square(0.5*s)))
...
def black(F, K, sigma, T, q):
    if q < 0:
        intrinsic = np.fabs(max((K-F), 0.0))
    else:
        intrinsic = np.fabs(max((F-K), 0.0))

    if (q*(F-K) > 0):
        return intrinsic + black(F, K, sigma, T, -q)

    return max(intrinsic, (np.sqrt(F)*np.sqrt(K))*normalised_black(np.log(F/K), sigma*np.sqrt(T), q))
...
def compute_f_lower_map_and_first_two_derivatives(x, s):
    ...
    fpp = PI_OVER_SIX * y/(s2*s) * Phi * (8*SQRT_THREE*s*ax + (3*s2*(s2-8)-8*x*x)*Phi/phi) * np.exp(2*y+0.25*s2)
    if (is_below_horizon(s)):
        fp = 1
        f = 0
    else:
        Phi2 = Phi*Phi
        fp = TWO_PI*y*Phi2*np.exp(y+0.125*s*s)
...
def inverse_f_lower_map(x, f):
    ...
    if is_below_horizon(f):
        ...
        return np.fabs(x/(SQRT_THREE*nd.inverse_norm_cdf(np.power(f/(TWO_PI_OVER_SQRT_TWENTY_SEVEN*np.fabs(x)), 1./3.))))


def inverse_f_upper_map(f):

    return -2.*nd.inverse_norm_cdf(f)
...
def unchecked_normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(beta, x, q, N):
    ...
    # Subtract intrinsic.
    if (q*x > 0):
        beta = np.fabs(max(beta-normalised_intrinsic(x, q), 0.))
        q = -q
    ...
    if (beta < b_c):
        ...
        if (beta < b_l):
            ...
            r_ll = rc.convex_rational_cubic_control_parameter_to_fit_second_derivative_at_right_side(0., b_l, 0., f_lower_map_l, 1., d_f_lower_map_l_d_beta, d2_f_lower_map_l_d_beta2, True)
            f = rc.rational_cubic_interpolation(beta, 0., b_l, 0.,
                                                f_lower_map_l, 1.,
        ...
        #     s_n+1  =  s_n  +  newton · [ 1 + halley·newton/2 ] / [ 1 + newton·( halley + hh3·newton/6 ) ]
        #
            while ((iterations < N) and (np.fabs(ds) > DBL_EPSILON*s)):
                if (ds*ds_previous < 0):
                    direction_reversal_count += 1
                if ((iterations > 0) and ((3 == direction_reversal_count) or
                                          (not ((s > s_left) and (s < s_right))))):
            ...
            r_lm = rc.convex_rational_cubic_control_parameter_to_fit_second_derivative_at_right_side(b_l, b_c, s_l, s_c, 1/v_l, 1/v_c, 0.0, False)

            s = rc.rational_cubic_interpolation(beta, b_l, b_c, s_l, s_c, 1/v_l, 1/v_c, r_lm)
            s_left = s_l
            s_right = s_c
        ...
        if v_c > DBL_MIN:
            ...
            b_h = normalised_black_call(x, s_h)  # TOCHECK about that s_c term
        if (beta <= b_h):
            v_h = normalised_vega(x, s_h)
            r_hm = rc.convex_rational_cubic_control_parameter_to_fit_second_derivative_at_left_side(b_c, b_h, s_c, s_h, 1/v_c, 1/v_h, 0.0, False)
            s = rc.rational_cubic_interpolation(beta, b_c, b_h, s_c, s_h,
                                                1/v_c, 1/v_h, r_hm)
            s_left = s_c
            s_right = s_h
        else:
            ...
            if ((d2_f_upper_map_h_d_beta2 > -SQRT_DBL_MAX) and (d2_f_upper_map_h_d_beta2 < SQRT_DBL_MAX)):
                r_hh = rc.convex_rational_cubic_control_parameter_to_fit_second_derivative_at_left_side(b_h, b_max, f_upper_map_h, 0., d_f_upper_map_h_d_beta, -0.5, d2_f_upper_map_h_d_beta2, True)
                f = rc.rational_cubic_interpolation(beta, b_h, b_max, f_upper_map_h, 0., d_f_upper_map_h_d_beta, -0.5, r_hh)
...
def implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(price, F, K, T, q, N):
    ...
    if (q*x > 0):
        price = np.fabs(max(price-intrinsic, 0.0))
        q = -q

    return unchecked_normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(price/(np.sqrt(F)*np.sqrt(K)), x, q, N)/np.sqrt(T)
